...
public class KPIForm extends DialogForm<KPI> {
    ...
    public KPIForm() {
        super(HyperLinks.KPI_DIALOG, 600, 500);
    }
    ...
    public void persist() throws Exception {
        if (model.getName() == null || model.getName().trim().isEmpty()) {
            UiUtils.showMessageBox("Missing KPI name", "KPI must have a name.");
            return;
        }
        if (model.getTargetValue() == null || model.getTargetValue() <= 0) {
            UiUtils.showMessageBox("Invalid target value", "Target value must be greater than 0.");
            return;
        }
        if (model.getMeasurementUnit() == null) {
            UiUtils.showMessageBox("Missing measurement unit", "KPI must have a measurement unit.");
            return;
        }
        if (model.getFrequency() == null) {
            UiUtils.showMessageBox("Missing frequency", "KPI must have a frequency.");
            return;
        }
        if (model.getStartDate() == null) {
            UiUtils.showMessageBox("Missing start date", "KPI must have a start date.");
            return;
        }
